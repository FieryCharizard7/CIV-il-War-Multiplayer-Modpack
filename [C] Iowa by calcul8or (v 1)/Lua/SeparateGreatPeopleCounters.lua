...
function Initialize_SGPC_Values(playerID, iX, iY)
	...
	if (player:IsAlive() and player:GetCivilizationType() == civilisationID and plot:IsCity() and plot:GetPlotCity():GetID() == player:GetCapitalCity():GetID()) then
...
function CityFounded_AdjustGreatPersonCounters(playerID, iX, iY)
	...
	local pointsPerGreatPersonEraAdjust = 0;
	local pointsPerGreatPersonSpeedAdj = 0;

	-- If the player has not founded their capital, update the starting Great Person progress
	if(player:IsAlive() and player:GetCivilizationType() == civilisationID and city:GetID() ~= player:GetCapitalCity():GetID()) then
	--if(not player:IsMinorCiv() and not player:IsBarbarian() and city:GetID() ~= player:GetCapitalCity():GetID()) then
		pointsPerGreatPersonSpeedAdj = (100 * GameInfo.GameSpeeds[Game:GetGameSpeedType()].GreatPeoplePercent) / 100;
		pointsPerGreatPersonEraAdjust = (100 * GameInfo.Eras[PreGame.GetEra()].GreatPeoplePercent) / 100;
		pointsPerGreatPerson = (pointsPerGreatPersonEraAdjust * pointsPerGreatPersonSpeedAdj) / 100;
