...
VALUES      ('LEADER_ROCKEFELLER',	 'POLICY_DIPLOMATIC_X',		'POLICY_FINANCIAL_X');

UPDATE Leaders 
SET Description = (CASE WHEN EXISTS(SELECT Type FROM Policies WHERE Type = 'POLICY_PHILOSOPHICAL_X' )
	THEN 'Rockefeller [ICON_HAPPINESS_1][ICON_GOLD]'
	ELSE 'TXT_KEY_LEADER_ROCKEFELLER' END) 
WHERE Type = 'LEADER_ROCKEFELLER';

CREATE TRIGGER CivIVTraitsNewYork
AFTER INSERT ON Leaders WHEN 'LEADER_ROCKEFELLER' = NEW.Type
BEGIN
	UPDATE Leaders 
	SET Description = (CASE WHEN EXISTS(SELECT Type FROM Policies WHERE Type = 'POLICY_PHILOSOPHICAL_X' )
		THEN 'Rockefeller [ICON_HAPPINESS_1][ICON_GOLD]'
...
SELECT	'CIVILIZATION_NEW_YORK',	ArtDefineTag, CultureType, SplashScreenTag, SoundtrackTag, UnitDialogueTag
FROM Civilization_JFD_CultureTypes WHERE CivilizationType = 'CIVILIZATION_AMERICA';

UPDATE Civilizations 
SET SoundtrackTag = 'JFD_Colonial'
WHERE Type = 'CIVILIZATION_NEW_YORK'
...
VALUES		('LEADER_ROCKEFELLER',	'FLAVOR_JFD_RELIGIOUS_INTOLERANCE',		6);
...
INSERT INTO Leader_Flavors
		(LeaderType,				FlavorType,					Flavor)
VALUES	('LEADER_ROCKEFELLER',	'FLAVOR_JFD_MERCENARY',		8);
